    ...
    def ingest_documents(self, documents: List[Dict]) -> float:
        ...
        for doc_id, doc_data in enumerate(documents):
            ...
            for i in range(len(sentences)):
                ...
                top_indices = np.argsort(similarities)[-self.top_k_per_sentence:][::-1]
                ...
                sentence_hash = hash(sentences[i])
                self.global_sentence_index[sentence_hash] = (doc_id, i)
            ...
            if isinstance(result_scores, list) and len(result_scores) > 0:
                ...
                for metric_name in metrics_to_extract:
                    if metric_totals[metric_name]:
                        ...
                        print(f"   ✅ {metric_name}: {avg_score:.3f} (avg of {len(metric_totals[metric_name])} samples)")
                    else:
                        scores[metric_name] = 0.0
                        print(f"   ⚠️  {metric_name}: no valid samples found")
...
def create_synthetic_dataset() -> Dict:
    """Create a synthetic dataset for evaluation if SQuAD fails"""
    ...
    documents = [
        {
            ...
            'question': 'How do machine learning algorithms identify patterns?',
            'id': 'synthetic_1'
        },
        {
            'context': """Climate change poses significant challenges to our planet's future sustainability. Rising global 
            ...
            'question': 'What are the effects of climate change on polar regions?',
            'id': 'synthetic_2'
        }
    ]
    ...
    ground_truths = [
        "Machine learning algorithms process vast amounts of data to identify complex patterns",
        "Climate change causes ice caps to melt at unprecedented rates across polar regions"
    ]
...
def run_comprehensive_evaluation(openai_api_key: str, num_samples: int = 50) -> Dict:
    ...
    document_aware_rag = DocumentAwareSemanticRAG(top_k_per_sentence=10)
    hierarchical_rag = HierarchicalSemanticRAG(top_k_per_sentence=10, cross_doc_k=5)

    # Initialize evaluator with rate limiting (20 samples max for safety)
    max_eval_samples = min(num_samples, 20)  # Limit to avoid rate limits
    evaluator = RAGEvaluator(
        openai_api_key,
        model="gpt-3.5-turbo",  # Use GPT-3.5 to avoid rate limits
        max_samples=max_eval_samples
    )

    # Load SQuAD dataset
    dataset_info = load_squad_dataset(num_samples)
    ...
    print(f"   Evaluation samples: {max_eval_samples} (rate-limit safe)")
    print("-" * 50)
...
def main(openai_api_key: str, num_samples: int = 50):
    ...
    print(f"📊 Using {num_samples} documents, evaluating on max 20 samples to avoid rate limits")

    try:
        # Run comprehensive three-way evaluation
        results = run_comprehensive_evaluation(openai_api_key, num_samples)
        ...
        print("   2. Reduce num_samples if hitting rate limits")
        print("   3. Wait a few minutes if you just hit rate limits")
        return None, None
...
if __name__ == "__main__":
    ...
    print("   • 50 documents maximum")
    print("   • 20 evaluation samples maximum")
    print("   • GPT-3.5 for evaluation (upgrade to GPT-4 for final results)")
    print("   • 30-second delays between system evaluations")
    print("   • Automatic retry logic")

    # You can just run this with your API key:
    results, report = main("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", num_samples=50)