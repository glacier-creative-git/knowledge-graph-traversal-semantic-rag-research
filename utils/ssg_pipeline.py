#!/usr/bin/env python3
"""
Semantic Similarity Graph Construction Pipeline
====================================

Specialized pipeline orchestrator for building semantic similarity graphs from raw data.
Handles data acquisition, embedding generation, similarity computation, theme extraction,
and semantic similarity graph construction (Phases 1-6).

This pipeline is focused exclusively on semantic similarity graph construction and does not
...
from utils.semantic_similarity_graph import SemanticSimilarityGraphBuilder
...
class SemanticSimilarityGraphPipeline:
    """Semantic similarity graph construction pipeline - Phases 1-6 only."""

    def __init__(self, config_path: str = "/Users/eric/Documents/Development/semantic-rag-chunking-research/config.yaml"):
        """Initialize the semantic similarity graph construction pipeline with configuration."""
        ...
        # Enhanced semantic similarity graph
        self.semantic_similarity_graph = None  # MultiGranularitySemanticSimilarityGraph instance
        self.ssg_stats = {}
    ...
    def build(self) -> Dict[str, Any]:
        """
        Build semantic similarity graph through phases 1-6.
        
        Returns:
            Dict containing experiment metadata and semantic similarity graph construction results
        """
        try:
            ...
            # Phase 6: Semantic Similarity Graph Construction
            if self.config['execution']['mode'] in ['full_pipeline']:
                if 'semantic_similarity_graph_construction' not in self.config['execution']['skip_phases']:
                    self._phase_6_semantic_similarity_graph_construction()
                else:
                    self.logger.info("⏭️  Skipping Phase 6: Semantic Similarity Graph Construction")

            self.logger.info("🎉 Semantic similarity graph construction pipeline completed successfully!")
            self.logger.info("💡 Semantic similarity graph is ready for use with retrieval algorithms and evaluation frameworks.")
            
            return {
                "experiment_id": self.experiment_id,
                "status": "completed",
                "execution_time": datetime.now() - self.start_time,
                "architecture": "multi_granularity_semantic_similarity_graph",
                "semantic_similarity_graph_path": str(Path(self.config['directories']['data']) / "semantic_similarity_graph.json"),
                "phases_completed": 6,
            ...
            if self.logger:
                self.logger.error(f"Semantic similarity graph construction pipeline failed: {str(e)}")
            else:
                print(f"Semantic similarity graph construction pipeline failed: {str(e)}")
            raise

    def _phase_1_setup_and_initialization(self):
        """Phase 1: Setup & Initialization"""
        print("🚀 Starting Semantic Similarity Graph Construction Pipeline")
        print("🌟 Architecture: Three-Tier Hierarchy (Document → Chunk → Sentence)")
        print("🎯 Scope: Phases 1-6 (Semantic Similarity Graph Construction Only)")
        print("=" * 70)
        ...
        self.logger.info(f"🎯 Phase 1 completed - Semantic Similarity Graph Experiment ID: {self.experiment_id}")
        self.logger.info(f"🏗️  Architecture: {self.config.get('experiment', {}).get('description', 'Multi-granularity semantic similarity graph system')}")
    ...
    def _phase_6_semantic_similarity_graph_construction(self):
        """Phase 6: Semantic Similarity Graph Assembly using pre-computed data from Phases 4 & 5."""
        self.logger.info("🏗️  Starting Phase 6: Semantic Similarity Graph Assembly")
        ...
        try:
            # Check if we should force recompute
            force_recompute = 'semantic_similarity_graph' in self.config['execution'].get('force_recompute', [])

            # Check cache
            ssg_path = Path(self.config['directories']['data']) / "semantic_similarity_graph.json"
            if not force_recompute and ssg_path.exists():
                self.logger.info("📂 Loading cached semantic similarity graph")
                self.semantic_similarity_graph = SemanticSimilarityGraph.load(str(ssg_path))
                # FIX: Load Phase 3 embeddings into cache after loading from JSON
                self.semantic_similarity_graph.load_phase3_embeddings(self.embeddings)
                self.ssg_stats = self.semantic_similarity_graph.metadata
            else:
                # Build fresh semantic similarity graph using assembly approach
                self.logger.info("🔨 Assembling fresh semantic similarity graph from pre-computed data")
                ssg_builder = SemanticSimilarityGraphBuilder(self.config, self.logger)

                # Fixed call with all required arguments:
                self.semantic_similarity_graph = ssg_builder.build_semantic_similarity_graph(
                    chunks=self.chunks,
                    multi_granularity_embeddings=self.embeddings,
                ...
                # Save semantic similarity graph
                self.logger.info(f"💾 Saving assembled semantic similarity graph to {ssg_path}")
                self.semantic_similarity_graph.save(str(ssg_path))

                self.ssg_stats = self.semantic_similarity_graph.metadata

            # Log semantic similarity graph statistics
            self.logger.info("📊 Semantic Similarity Graph Assembly Statistics:")
            self.logger.info(f"   Architecture: {self.ssg_stats.get('architecture', 'phase6_assembly')}")

            if 'theme_bridge_stats' in self.ssg_stats:
                bridge_stats = self.ssg_stats['theme_bridge_stats']
            ...
            self.logger.info(f"   Build time: {self.ssg_stats.get('build_time', 0):.2f}s")

            # Initialize the new retrieval engine with semantic similarity graph
            self.logger.info("🎯 Initializing enhanced retrieval engine")
            self.retrieval_engine = create_retrieval_engine(
                semantic_similarity_graph=self.semantic_similarity_graph,
                config=self.config,
                logger=self.logger
            )
            ...
            try:
                ...
                self.retrieval_stats = {
                    ...
                    'semantic_similarity_graph_enabled': True,
            ...
            self.logger.info("✅ Phase 6 Semantic Similarity Graph Assembly completed successfully")
            self.logger.info("🚀 Semantic similarity graph is ready for retrieval and evaluation!")
    ...
    def _initialize_experiment_tracker(self):
        ...
        self.experiment_id = f"ssg_{self.config['experiment']['name']}_{timestamp}_{short_uuid}"

        print(f"🔬 Semantic Similarity Graph Experiment ID: {self.experiment_id}")
    ...
    def _initialize_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger('semantic_similarity_graph_pipeline')
        ...
        # Log initial info
        self.logger.info(f"Semantic similarity graph construction logging initialized for experiment: {self.experiment_id}")
...
def main():
    """Main entry point for the semantic similarity graph construction pipeline."""
    try:
        # Initialize and run semantic similarity graph construction pipeline
        pipeline = SemanticSimilarityGraphPipeline()
        results = pipeline.build()

        print("\n" + "=" * 70)
        print("🎉 Semantic Similarity Graph Construction Pipeline completed successfully!")
        print(f"🌟 Architecture: {results.get('architecture', 'multi_granularity_semantic_similarity_graph')}")
        print(f"📋 Experiment ID: {results['experiment_id']}")
        print(f"⏱️  Execution time: {results['execution_time']}")
        print(f"📁 Semantic similarity graph saved: {results['semantic_similarity_graph_path']}")
        print(f"🔍 Phases completed: {results['phases_completed']}")
        print(f"➡️  Next steps: {results['next_steps']}")

    except Exception as e:
        print(f"\n❌ Semantic similarity graph construction pipeline failed: {e}")
        sys.exit(1)
