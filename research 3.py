...
class TraditionalRAG:
    ...
    def __init__(self, chunk_size: int = 400, overlap: int = 50, embedding_model: str = "all-MiniLM-L6-v2"):
...
class HierarchicalSemanticRAG(DocumentAwareSemanticRAG):
    ...
    def __init__(self, top_k_per_sentence: int = 10, cross_doc_k: int = 5, embedding_model: str = "all-MiniLM-L6-v2"):
        super().__init__(top_k_per_sentence, embedding_model)
...
def load_squad_dataset(num_samples: int = 500, version: str = "2.0") -> Dict:
    """Load SQuAD dataset - can choose version 1.1 or 2.0"""

    dataset_name = "squad_v2" if version == "2.0" else "squad"
    print(f"📁 Loading SQuAD {version} dataset ({num_samples} samples)...")
    print(f"   {'🔥 SQuAD 2.0: Includes unanswerable questions (harder!)' if version == '2.0' else '📊 SQuAD 1.1: All questions have answers (easier)'}")

    try:
        # Load SQuAD dataset - version 2.0 is harder with unanswerable questions
        ...
        for item in squad:
            ...
            if item['answers']['text']:
                ...
                # SQuAD 2.0 has unanswerable questions
                ground_truths.append("No answer can be determined from the given context")

        print(f"   ✅ Loaded {len(documents)} SQuAD {version} documents")
        ...
        print(f"   🔄 Falling back to long document dataset...")
...
def run_comprehensive_evaluation(openai_api_key: str, num_samples: int = 50, dataset_type: str = "long_docs",
                                ...
                                embedding_model: str = "all-MiniLM-L6-v2", squad_version: str = "2.0") -> Dict:
    ...
    embedding_info = {
        ...
        "text-embedding-ada-002": "1536 dimensions (OpenAI, high quality)",
        "jina-embeddings-v2-base-en": "768 dimensions (specialized for retrieval)"
    }
    print(f"🧠 Embedding Model: {embedding_model}")
    print(f"   {embedding_info.get(embedding_model, 'Custom model')}")
    ...
    # Initialize systems with specified embedding model
    traditional_rag = TraditionalRAG(chunk_size=300, embedding_model=embedding_model)
    document_aware_rag = DocumentAwareSemanticRAG(top_k_per_sentence=doc_aware_k, embedding_model=embedding_model)
    hierarchical_rag = HierarchicalSemanticRAG(top_k_per_sentence=doc_aware_k, cross_doc_k=cross_doc_k, embedding_model=embedding_model)
    ...
    # Load dataset with SQuAD version support
    ...
    elif dataset_type == "squad" and squad_version == "2.0":
        print(f"   🔥 SQuAD 2.0: Hardest benchmark with unanswerable questions!")
    else:
...
def main(openai_api_key: str, num_samples: int = 50, dataset_type: str = "long_docs",
         use_gpt4: bool = False, aggressive_k: bool = False,
         embedding_model: str = "all-MiniLM-L6-v2", squad_version: str = "2.0"):
    ...
    dataset_descriptions = {
        ...
        "squad": f"SQuAD {squad_version} contexts"
    }
    ...
    try:
        ...
        results = run_comprehensive_evaluation(openai_api_key, num_samples, dataset_type, use_gpt4, aggressive_k, embedding_model, squad_version)
...
if __name__ == "__main__":
    ...
    print('   # Test 1: SQuAD 2.0 + Better embeddings + Aggressive')
    # results1 = main("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", dataset_type="squad", aggressive_k=True, embedding_model="all-mpnet-base-v2")
    print()
    print('   # Test 2: Cross-document dataset + Aggressive')
    # results2 = main("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", dataset_type="cross_docs", aggressive_k=True)
    print()
    print('   # Test 3: Long documents + Better embeddings')
    results3 = main("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", dataset_type="long_docs", embedding_model="all-mpnet-base-v2")
    print()
    print("💰 FINAL PUBLICATION TESTS (GPT-4 - only when ready!):")
    print('   # Use these only after perfecting with GPT-3.5')
    print('   final = main("your-key", dataset_type="squad", use_gpt4=True, aggressive_k=True, embedding_model="all-mpnet-base-v2")')
    print()
    print("🧪 EMBEDDING MODEL OPTIONS:")
    print("   'all-MiniLM-L6-v2'     - 384 dims (current, fast)")
    print("   'all-mpnet-base-v2'    - 768 dims (better quality, recommended!)")
    print("   'jina-embeddings-v2-base-en' - 768 dims (specialized for retrieval)")
    print()
    print("📊 DATASET OPTIONS:")
    print("   dataset_type:")
    print("     'squad'      - SQuAD benchmark (specify version with squad_version='2.0')")
    print("     'cross_docs' - BEST for testing hierarchical advantages")
    print("     'long_docs'  - BEST for testing document-aware advantages")
    print("   squad_version: '1.1' (easier) | '2.0' (harder, recommended)")
    print()
    print("🎯 RECOMMENDED UPGRADE PATH:")
    print("   1. Test current approach with better embeddings (all-mpnet-base-v2)")
    print("   2. Verify on SQuAD 2.0 (hardest benchmark)")
    print("   3. Test cross-document capabilities")
    print("   4. Only use GPT-4 for final publication results")
    print()
    print("🔬 CURRENT FINDINGS:")
    print("   ✅ Semantic graphs consistently WIN on retrieval metrics (precision/recall)")
    print("   ⚠️  Faithfulness scores suggest answer generation needs improvement")
    print("   💡 Try better embedding models - 384 dims may be limiting performance")