...
class SQuADSemanticGraphAnalyzer:
    ...
    def analyze_squad_question(self, question: str = None, contexts: List[Dict] = None) -> Tuple[List[str], List[TraversalStep], Dict]:
        ...
        # Perform detailed retrieval and analysis with two-phase approach
        retrieved_texts, traversal_steps = self._detailed_retrieve(question, top_k=15)

        print(f"\n🎯 Two-phase retrieval complete!")
        ...
        # Add simple reranking success metrics
        query_embedding = self.model.encode([question])

        # Get similarities for final retrieved texts
        final_similarities = []
        for text in retrieved_texts:
            # Find the step corresponding to this text
            for step in traversal_steps:
                if step.sentence_info.text.strip() == text.strip():
                    query_similarity = float(np.dot(query_embedding[0], step.sentence_info.embedding))
                    final_similarities.append(query_similarity)
                    break

        analysis['reranking_analysis'] = {
            'top_similarity': max(final_similarities) if final_similarities else 0.0,
            'bottom_similarity': min(final_similarities) if final_similarities else 0.0,
            'similarity_range': max(final_similarities) - min(final_similarities) if final_similarities else 0.0,
            'num_results': len(final_similarities)
        }

        print(f"📈 Final Results Quality:")
        if final_similarities:
            print(f"   Best result similarity: {max(final_similarities):.3f}")
            print(f"   Worst result similarity: {min(final_similarities):.3f}")
            print(f"   Similarity range: {max(final_similarities) - min(final_similarities):.3f}")
    ...
    def _detailed_retrieve(self, query: str, top_k: int = 5) -> Tuple[List[str], List[TraversalStep]]:
        ...
        # PHASE 1: Semantic Graph Discovery (what we just did)
        print(f"\n🔍 Phase 1 - Discovery via Semantic Graph Traversal")
        print(f"Found {len(all_traversal_steps)} related sentences")

        # Show discovery order with direct query similarities
        print(f"\n📋 DISCOVERY ORDER:")
        print(f"Prompt: {query}")
        print("-" * 80)

        discovery_order_steps = sorted(all_traversal_steps, key=lambda x: x.step_number)
        discovery_similarities = []

        for step in discovery_order_steps:
            # Calculate direct similarity to original query
            direct_similarity = np.dot(query_embedding[0], step.sentence_info.embedding)
            discovery_similarities.append(direct_similarity)

            # Format output
            step_label = "Anchor Point" if step.connection_type == 'anchor' else f"Step {step.step_number}"
            connection_info = f"({step.connection_type.replace('_', ' ')})" if step.connection_type != 'anchor' else ""
            text_preview = step.sentence_info.text[:100] + "..." if len(step.sentence_info.text) > 100 else step.sentence_info.text

            print(f"{step_label}: (Similarity: {direct_similarity:.3f}) {connection_info} {text_preview}")

        # PHASE 2: Direct Query Re-ranking
        print(f"\n🎯 Phase 2 - Reranking by Direct Query Similarity")
        ...
        for step in all_traversal_steps:
            ...
            reranked_step = TraversalStep(
                ...
                relevance_score=float(direct_similarity),  # Convert to Python float for JSON
        ...
        # Show reranked order
        print(f"\n📋 RERANKED ORDER (by direct query similarity):")
        print(f"Prompt: {query}")
        print("-" * 80)

        reranked_similarities = []
        for i, step in enumerate(reranked_steps):
            reranked_similarities.append(step.relevance_score)

            # Format output
            original_step_label = "Anchor Point" if step.connection_type == 'anchor' else f"Step {step.step_number}"
            connection_info = f"({step.connection_type.replace('_', ' ')})" if step.connection_type != 'anchor' else ""
            text_preview = step.sentence_info.text[:100] + "..." if len(step.sentence_info.text) > 100 else step.sentence_info.text

            print(f"Rank {i+1} [{original_step_label}]: (Similarity: {step.relevance_score:.3f}) {connection_info} {text_preview}")
        ...
        print(f"\n✅ Final: Returning top {len(unique_texts)} unique, reranked results")
        print(f"📊 Top result similarity: {unique_reranked_steps[0].relevance_score:.3f}")
        print(f"📊 Lowest result similarity: {unique_reranked_steps[-1].relevance_score:.3f}")

        # Return both the reranked texts and ALL traversal steps (for visualization)
    ...
    def _analyze_traversal_patterns(self, traversal_steps: List[TraversalStep]) -> Dict:
        ...
        analysis = {
            ...
            'duplicate_rate': float(duplicate_rate),  # Convert to Python float
            'context_duplicate_rate': float(context_duplicate_rate),  # Convert to Python float
            ...
            'connection_type_percentages': {
                conn_type: float((count / total_steps) * 100)  # Convert to Python float
            ...
            'average_paragraph_jump': float(np.mean(paragraph_jumps)) if paragraph_jumps else 0.0,  # Convert to Python float
            'max_paragraph_jump': int(max(paragraph_jumps)) if paragraph_jumps else 0,  # Convert to Python int
            'paragraph_locality_score': float((connection_counts['same_paragraph'] / total_steps) * 100),  # Convert to Python float
            'cross_document_rate': float((connection_counts['cross_document'] / total_steps) * 100),  # Convert to Python float
            'semantic_diversity_score': float(len(set(step.sentence_info.doc_id for step in traversal_steps)) / len(self.selected_contexts) * 100)  # Convert to Python float
        }
...
class SemanticGraphVisualizer:
    ...
    def create_3d_traversal_visualization(self, query: str, traversal_steps: List[TraversalStep],
        ...
        fig.update_layout(
            title=f"SQuAD Semantic Graph Discovery Process (Forward-Looking Windows)<br>" +
                  f"Query: '{query[:80]}...'<br>" +
                  f"Contexts: {len(self.rag_system.selected_contexts)} | " +
                  f"Steps: {len(sorted_steps)} | " +
                  f"Cross-doc jumps: {len([s for s in sorted_steps if s.connection_type == 'cross_document'])}<br>" +
                  f"<i>Note: Shows discovery order; final results are reranked by direct query similarity</i>",
            scene=dict(
...
def run_squad_analysis(embedding_model: str = "all-MiniLM-L6-v2",
    ...
    print(f"🎯 Two-Phase Retrieval: Discovery via semantic graph + Reranking by direct query similarity")
    print("=" * 50)
    ...
    # Reranking analysis
    reranking = analysis.get('reranking_analysis', {})
    if reranking:
        print(f"\n📈 Query Reranking Results:")
        print(f"Best result similarity: {reranking.get('top_similarity', 0):.3f}")
        print(f"Worst result similarity: {reranking.get('bottom_similarity', 0):.3f}")
        print(f"Similarity range: {reranking.get('similarity_range', 0):.3f}")
        print(f"Number of results: {reranking.get('num_results', 0)}")

        if reranking.get('top_similarity', 0) > 0.7:
            print(f"   ✅ Excellent top result! Very high similarity to query.")
        elif reranking.get('top_similarity', 0) > 0.5:
            print(f"   ✅ Good top result similarity.")
        else:
            print(f"   ⚠️  Top result has modest similarity to query.")
    ...
    # Save results with reranking information
    window_suffix = "_sliding_window" if use_sliding_window else "_single_sentence"

    # Enhanced filename to indicate two-phase approach
    analysis_suffix = "_2phase" + window_suffix

    # Save visualizations
    viz_3d_filename = f"squad_3d_traversal_{analysis_type}{analysis_suffix}.html"
    fig_3d.write_html(os.path.join(output_dir, viz_3d_filename))
    print(f"🎨 3D visualization saved to {output_dir}/{viz_3d_filename}")

    viz_patterns_filename = f"squad_pattern_analysis_{analysis_type}{analysis_suffix}.html"
    ...
    # Save analysis
    filename = f"squad_analysis_{analysis_type}{analysis_suffix}.json"
    ...
    retrieved_filename = f"retrieved_texts_{analysis_type}{analysis_suffix}.txt"
    with open(os.path.join(output_dir, retrieved_filename), 'w') as f:
        f.write(f"Question: {question}\n\n")
        f.write("="*60 + "\n")
        f.write("RETRIEVED TEXTS (Reranked by Direct Query Similarity)\n")
        f.write("="*60 + "\n\n")

        # Add reranking info
        reranking = analysis.get('reranking_analysis', {})
        if reranking:
            f.write(f"Best result similarity: {reranking.get('top_similarity', 0):.3f}\n")
            f.write(f"Worst result similarity: {reranking.get('bottom_similarity', 0):.3f}\n")
            f.write(f"Similarity range: {reranking.get('similarity_range', 0):.3f}\n\n")
    ...
    context_filename = f"contexts_{analysis_type}{analysis_suffix}.txt"
...
if __name__ == "__main__":
    ...
    print("\n💡 Tips:")
    print("   • Visualization shows discovery process via semantic graph traversal")
    print("   • Final results are reranked by direct query similarity (Phase 2)")
    print("   • Cross-document jumps show as solid purple lines")
    ...
    print("   • Look for reranking improvement % in the analysis output!")
